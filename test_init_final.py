		...
		if channel != '':			
			...
			for i in range(fixed_bossNum):
				...
				if fixed_bossTime[i] <= now :
					...
					embed = discord.Embed(
							description= "```" + fixed_bossData[i][0] + ' 타임 ' + fixed_bossData[i][4] + "```" ,
							color=0x00ff00
							)
			...
			for i in range(bossNum):
				...
				if bossTime[i] <= now :
					...
					embed = discord.Embed(
							description= "```" + bossData[i][0] + ' 타임 ' + bossData[i][4] + "```" ,
							color=0x00ff00
							)
					await client.get_channel(channel).send(embed=embed, tts=False)
				...
				if bossMungFlag[i] == True:
					if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
						if basicSetting[2] != '0':
							################ 미입력 보스 ################
							if bossData[i][2] == '0':
								await client.get_channel(channel).send("```" +  bossData[i][0] + ' 다음 타임 예약이 아직 안되었습니다.```', tts=False)
							...
							################ 멍 보스 ################
								...
								await client.get_channel(channel).send("```" + bossData[i][0] + ' 멍인가봐요..```')
...
async def on_ready():
	...
	await client.change_presence(status=discord.Status.idle, activity=discord.Game(name="7섭 보탐언냐♡", type=1))
...
while True:
	...
	async def on_message(msg):
		...
		if client.get_channel(channel) != msg.channel :
			...
			if message.content.startswith('/보스')  or message.content.startswith('/보탐') or message.content.startswith('/보스탐') or message.content.startswith('/qhtm') or message.content.startswith('/?'):
				...
				for i in range(bossNum):
					if bossTimeString[i] == '99:99:99' and bossMungFlag[i] != True :
						...
						if bossMungFlag[i] == True :
							...
							aa.append(tmp_bossTime[i].strftime('%H:%M'))  #output_bossData[2] : 시간(00:00:00)  *초 제거 희망시 '%H:%M:%S' -> '%H:%M'
				...
				for i in range(fixed_bossNum):
					...
					aa.append(fixed_bossTime[i].strftime('%H:%M'))    #output_bossData[2] : 시간(00:00:00)      *초 제거 희망시 '%H:%M:%S' -> '%H:%M'
					aa.append('@')                                       #output_bossData[3] : @
			...
			if message.content == '!@':
				...
				for i in range(bossNum):
					if bossTimeString[i] == '99:99:99' and bossMungFlag[i] != True :
						...
						if bossMungFlag[i] == True :
							aa.append(tmp_bossTime[i])                       #output_bossData[1] : 시간
							aa.append(tmp_bossTime[i].strftime('%H:%M'))  #output_bossData[2] : 시간(00:00:00)   *초 제거 희망시 *초 제거 희망시 '%H:%M:%S' -> '%H:%M'
							aa.append('-')	                                 #output_bossData[3] : -
						else :
							aa.append(bossTime[i])                           #output_bossData[1] : 시간
							aa.append(bossTime[i].strftime('%H:%M'))      #output_bossData[2] : 시간(00:00:00)   *초 제거 희망시 *초 제거 희망시 '%H:%M:%S' -> '%H:%M'
							aa.append('+')	                                 #output_bossData[3] : +
						aa.append(bossData[i][2])                            #output_bossData[4] : 멍/미입력 보스
						aa.append(bossMungCnt[i])	                         #output_bossData[5] : 멍/미입력횟수
						aa.append(bossData[i][6])	                         #output_bossData[6] : 메세지
				...
				fixed_information = ''
				for i in range(fixed_bossNum):
						tmp_timeSTR = fixed_bossTime[i].strftime('%H:%M')  #*초 제거 희망시 '%H:%M:%S' -> '%H:%M'
